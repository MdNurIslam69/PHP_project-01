<title><?php echo isset($title) ? $title : "Shifted Orders | Imran_Store"; ?></title>

<?php
require_once('header.php');
require_once('sidebar.php');

// Update status on the ORDERS table, not order_items
...
// Select all order items with orders that have status 'Shifted'
$query = "
SELECT 
    ...
    <div class="">
        <div class="col-12 mt-4">
            ...
            <div>
                ...
                <table class="table border table-bordered table-striped mb-4" id="pendingOrdersTable"
                    ...
                    <thead>
                        <tr class="bg-info text-white">
                            ...
                            <th class="text-center align-middle p-0" style="border: 2px solid black">Product Image
                            </th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center align-middle" style="border: 1px solid black">
                                <?= $row['user_name'] ?></td>
                            <td class="align-middle" style="border: 1px solid black"><?= $row['product_name'] ?>
                            </td>
                            <td class="text-center align-middle p-0" style="border: 1px solid black">
                                <img src="../assets/img/products/<?= $row['images'] ?>"
                                    alt="<?= $row['product_name'] ?>" width="80" height="80"
                                    class=" object-fit-contain">
                            </td>
                            <td class="text-center align-middle" style="border: 1px solid black">
                                <?= $row['quantity'] ?></td>
                            <td class="text-center align-middle" style="border: 1px solid black; font-weight: bold;">
                            ...
                            <td class="align-middle" style="border: 1px solid black"><?= $row['address'] ?></td>
                            <td class="text-center align-middle" style="border: 1px solid black">
                                ...
                                <form action="" method="POST">
                                    <select name="status" class="form-select mb-2" required>
                                        <option value="Pending" <?= $row['status'] == 'Pending' ? 'selected' : '' ?>>
                                            Pending</option>
                                        <option value="Shifted" <?= $row['status'] == 'Shifted' ? 'selected' : '' ?>>
                                            Shifted</option>
                                        <option value="Success" <?= $row['status'] == 'Success' ? 'selected' : '' ?>>
                                            Success</option>
                                        <option value="Refunded" <?= $row['status'] == 'Refunded' ? 'selected' : '' ?>>
                                            Refunded</option>
                ...
                <?php } else { ?>
                <br><br><br><br>
                <p class="text-center text-danger" style="font-size: 30px;">No Shifted orders
                    found</p>
